    ...
    public static ArrayList<Package> loadPackages(File file) {
        ...
        try {
    	    ...
    	    while (input.hasNextLine()) {
    	        ...
    	        packages.add(new Package(info[0], info[1], Double.parseDouble(info[2]),
                        Double.parseDouble(info[3]), new ShippingAddress(info[4], info[5],
                        info[6], info[7], Integer.parseInt(info[8]))));
            }
        } catch (FileNotFoundException e) {
    	    return packages;
        }
    ...
    public static void saveVehicles(File file, ArrayList<Vehicle> vehicles) {
        try {
            ...
            for (Vehicle i : vehicles) {
                String[] info = {i.getType(),
                        i.getLicensePlate(), String.valueOf(i.getMaxWeight())};
                pw.println(String.join(",", info));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void savePackages(File file, ArrayList<Package> packages) {
        try {
            ...
            for (Package i : packages) {
                String[] info = {i.getID(), i.getProduct(),
                String.valueOf(i.getWeight()), String.valueOf(i.getPrice()),
                i.getProduct(), i.getDestination().getName(), i.getDestination().getAddress(),
                i.getDestination().getCity(), i.getDestination().getState(),
                        String.valueOf(i.getDestination().getZipCode())};
                pw.println(String.join(",", info));
            }
        } catch (IOException e) {
            e.printStackTrace();
